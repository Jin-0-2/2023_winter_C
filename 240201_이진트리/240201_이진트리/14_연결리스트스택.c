...
void mystack_init(MyStack* p)
{
	mylist_init(p);
}

int mystack_push(MyStack* p, int value)
{
	return mylist_pushback(p, value);
}

void mystack_print(const MyStack* p)
{
	mylist_printall(p);
}

int mystack_pop(MyStack* p)
{
	return mylist_deleteback(p);
}
//--------------------------------------------------------------
...
int main()
{
	...
	for (int i = 0; i < 5; i++)
		mystack_push(&mstack, (i + 1) * 10);

	mystack_print(&mstack);

	printf("POP 2번\n");
	printf("[POP : %d]\n", mystack_pop(&mstack)); //50
	mystack_print(&mstack);
	printf("[POP : %d]\n", mystack_pop(&mstack)); //40
	mystack_print(&mstack);
